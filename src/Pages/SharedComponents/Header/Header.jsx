        ...
        <div className='bg-accent text-base-100 relative'>
            <div className='container mx-auto flex justify-between items-center'>
                ...
                <div className={`absolute z-50 py-6 space-y-3 font-medium border border-primary flex flex-col text-lg justify-center items-center bg-base-100 text-accent w-full ${openNav ? "top-[70px]" : "-top-96"} left-0`}>
                    <NavLink onClick={() => setOpenNav(!openNav)} to="/" className={({ isActive }) => isActive ? 'active' : ''}>Home</NavLink>
                    <NavLink onClick={() => setOpenNav(!openNav)} to="/allToys" className={({ isActive }) => isActive ? 'active' : ''}>All Toys</NavLink>
                    {
                        user ? <>
                            <NavLink onClick={() => setOpenNav(!openNav)} to="/sellerOwnToys" className={({ isActive }) => isActive ? 'active' : ''}>My Toys</NavLink>
                            <NavLink onClick={() => setOpenNav(!openNav)} to="/addToy" className={({ isActive }) => isActive ? 'active' : ''}>Add Toy</NavLink>
                        </> : ''
                    }
                    <NavLink onClick={() => setOpenNav(!openNav)} to="/blog" className={({ isActive }) => isActive ? 'active' : ''}>Blog</NavLink>
                </div>
                <div className='lg:hidden'>
                    {
                        user ?
                            ...
                            </div> : <NavLink to="/login" className={({ isActive }) => isActive ? 'btn btn-primary capitalize px-10 rounded-full' : 'btn btn-primary capitalize px-10 rounded-full'}>Login</NavLink>
                    }
                </div>
            </div>
        </div >
